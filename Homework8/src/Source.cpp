...
int main() {
	...
	*/
	...
	/// hw8
	Shader bezierShader("bezierShader.vs", "bezierShader.frag");
	bool animation_flag = false;
	float animation_t0 = glfwGetTime()*1000;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// ========= ImGui =======================================================================================
		{
			...
			if (ImGui::BeginMenuBar())
			{
				if (ImGui::BeginMenu("File"))
				{
					...
					if (ImGui::MenuItem("homework8-bezier")) {
						choose_index = 8;
						homework4 = false;
						homework5_projection = false;
						moon = false;
			...
			if (ImGui::Button("Animation")) {
				...
				cout << "animation_t0 " << animation_t0 << endl;
			}
		...
		if(choose_index == 8) {
			...
			glfwSetMouseButtonCallback(window, mouse_callback_bezier);
			
			processInputBezier(window, animation_t0);
			...
			bezierShader.Use();
			glUniform1i(glGetUniformLocation(bezierShader.Program, "animation"), 0);
			glUniformMatrix4fv(glGetUniformLocation(bezierShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
			glUniformMatrix4fv(glGetUniformLocation(bezierShader.Program, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
			glUniformMatrix4fv(glGetUniformLocation(bezierShader.Program, "view"), 1, GL_FALSE, glm::value_ptr(view));
			...
			if (draw_size1 > 1) { // 注意要draw_size1>1即有两个点才开始画边连线
				...
				if (delta_t<= 1 && delta_t >= 0) {
					...
					glPointSize(20);
					glUniform1i(glGetUniformLocation(bezierShader.Program, "animation"), 1);
...
float B(int i, int n, float t) {
	...
	result =  factor(n, i)* pow(t, i) * pow((1 - t), n - i);
